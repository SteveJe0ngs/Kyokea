...
class ConvexHull3D():
    ...
    '''
    def __init__(self, pts, run=True, preproc=False, make_frames=False, frames_dir='../python/output/frames/'):
    ...
    def generateImage(self, newPt=None, show=False, profs_w=None):
        ...
        if profs_w is not None:
            ...
            for profs in profs_w.keys():
                x.append(profs_w[profs]['x'])
                y.append(profs_w[profs]['y'])
                z.append(profs_w[profs]['z'])
