...
with open('src/python/output/jsons/user.json', 'w', encoding='utf-8') as make_file:
    json.dump(file_data, make_file, ensure_ascii=False, indent='\t')

#pts.json 불러오기
with open('src/python/output/jsons/pts.json', 'r', encoding='UTF-8') as f:
    profs_pts = json.load(f)

#convexhull 생성
pts = np.array(profs_pts)
Hull = ConvexHull3D(pts, run=True, preproc=False,
                    make_frames=True, frames_dir='src/python/output/frames/')
vertices = Hull.DCEL.vertexDict.values()
pts = Hull.getPts()
hull_vertices = pts[Hull.getVertexIndices()]
faces = [[list(v.p()) for v in face.loopOuterVertices()]
...
image_folder = 'src/python/output/frames'
video_name = 'src/python/output/videos/output.mp4'
